/**
 * Vendored from MangoLion/stretchystudio, MIT license.
 * Source module: src/io/live2d/moc3writer.js
 * Local use: experimental static-quad Live2D runtime bundle generation.
 *
...
function buildSectionData(input) {
  const { project, regions, atlasSize } = input;
  ...
  // --- ArtMesh sections ---
  sections.set('art_mesh.ids', meshParts.map(p => p.id));
  ...
  sections.set('art_mesh.parent_part_indices', meshParts.map(p => {
    let cur = p.parent ? project.nodes.find(n => n.id === p.parent) : null;
    while (cur) {
